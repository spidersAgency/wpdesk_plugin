...
<div class="wrap wpdesk">
	<?php /* screen_icon(); */ ?>
	<h1><?php _e( 'WP Desk', 'wpdesk-helper' ); ?><a href="<?php echo admin_url( 'admin.php?page=wpdesk-helper&refresh=1' ); ?>" class="page-title-action"><?php echo __( 'Refresh', 'wpdesk-helper' ); ?></a></h1>
		...
		<?php
				...
				foreach ($wp_plugins as $wp_key => $wp_plugin) {
					if ($wp_plugin['PluginURI'] == $plugin['url']) {
						...
						if (is_plugin_active($wp_key)) {
							$plugin_state = 'active';
							if ( is_object( $wpdesk_installed_plugins[$wp_key] ) && get_option( $wpdesk_installed_plugins[$wp_key]->activated_key, '0' ) != 'Activated') {
			...
			<div class="plugin-card">
				<div class="plugin-card-top">
					...
					<div class="">
						<a href="<?php echo $plugin['url'] . $utm_link; ?>" target="_blank"><img src="<?php echo str_replace('http://','//',$plugin['image']); ?>"></a>
				...
				<div class="plugin-card-bottom">
					<div class="vers column-rating">
						<?php if ($plugin_state == 'not present') : ?>
							<a class="button button-primary" href="<?php echo $plugin['url'] . $utm_link; ?>" target="_blank">
								<?php echo __( 'Download plugin', 'wpdesk-helper' ); ?>
							</a>
						<?php endif; ?>
						<?php if ($plugin_state == 'not active') : ?>
							<a class="button" href="<?php echo admin_url('plugins.php#'.sanitize_title($wp_plugin['Name'])); ?>">
								<?php echo __( 'Activate plugin', 'wpdesk-helper' ); ?>
							</a>
						<?php endif; ?>
						<?php if ($plugin_state == 'not active key') : ?>
							<a class="button" href="<?php echo admin_url( 'admin.php?page=wpdesk-licenses' ); ?>">
								<?php echo __( 'Activate key', 'wpdesk-helper' ); ?>
							</a>
						<?php endif; ?>
						<?php if ($plugin_state == 'active key' && is_object( $wpdesk_installed_plugins[$wp_key] ) && $wpdesk_installed_plugins[$wp_key]->config_uri && $wpdesk_installed_plugins[$wp_key]->config_uri != '') : ?>
							<a class="button" href="<?php echo $wpdesk_installed_plugins[$wp_key]->config_uri; ?>">
					...
					<div class="column-updated">
						<?php if ($plugin_state == 'active key' && $plugin_state != 'not present') : ?>
							<?php if ($plugin['version'] != $wp_plugin['Version']) : ?>
								<a class="update-now button" href="<?php echo admin_url('plugins.php#'.sanitize_title($wp_plugin['Name'])); ?>">
