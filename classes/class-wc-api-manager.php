...
if (!class_exists('WPDesk_API_Manager')) :
	...
	class WPDesk_API_Manager {
		...
		*/
		...
		public function __construct( $upgrade_url, $version, $name, $product_id, $menu_title, $title, $plugin_file, $plugin_dir, $config_uri ) {
			global $wpdesk_installed_plugins;
			if (!$wpdesk_installed_plugins) {
				$wpdesk_installed_plugins = array();
			}
...
//				new WPDesk_API_MENU( $this );
		...
		public function inactive_notice() { ?>
			...
			<?php if ( isset( $_GET['page'] ) && 'wpdesk-licenses' == $_GET['page'] ) return; ?>
			<div class="update-nag">
				<?php printf( __( 'The %s%s%s License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', $this->text_domain ), '<strong>', $this->software_product_id, '</strong>', '<a href="' . esc_url( admin_url( 'admin.php?page=wpdesk-licenses' ) ) . '">', '</a>' ); ?>
			</div>
			<?php
		}
