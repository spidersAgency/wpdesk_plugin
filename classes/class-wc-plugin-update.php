...
if (!class_exists('WPDesk_Update_API_Check')) :
	...
	class WPDesk_Update_API_Check {
		...
		public function plugin_information( $args ) {
			...
			$request = wp_safe_remote_get( $target_url, array('timeout' => 30, 'sslverify' => false ) );
			...
			if ( is_object( $response ) ) {
				if ( isset( $response->sections ) ) {
					$response->sections['description'] = apply_filters( 'the_content', ( isset( $response->sections['description'] ) ? $response->sections['description'] : '' ) );
